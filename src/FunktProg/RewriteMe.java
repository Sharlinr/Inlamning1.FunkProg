...
public class RewriteMe {
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
                    return questions.stream().filter(s->s.getCategory().equals(category))
                            .map(s->s.getQuestionString()).collect(Collectors.toList());
    }
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        return questions.stream().flatMap(s->s.getAllAnswers().stream()).distinct()
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) questions.stream().filter(s->s.getAllAnswers()
    ...
    public static void main(String[] args){
        RewriteMe rm = new RewriteMe();

        System.out.println(rm.getAmountOfQuestionsInDatabase());

        System.out.println(rm.getAmountOfQuestionsForACertainCategory(Category.HISTORY));


        rm.getListOfAllQuestions().forEach(System.out::println);

        System.out.println(rm.getAllQuestionStringsBelongingACategory(Category.HISTORY));

        rm.getAllAnswerOptionsDistinct().forEach(System.out::println);

        System.out.println(rm.isThisAnAnswerOption("väte"));

        System.out.println(rm.getAnswerCandidateFrequncy("hej"));
